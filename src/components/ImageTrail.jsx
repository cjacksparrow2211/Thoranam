...
export default function ImageTrail({ items = [], variant = 1, children }) {
  ...
  return (
    <div className="w-full h-full relative rounded-lg bg-transparent overflow-hidden" ref={containerRef}>
      {children}
      {items.map((url, i) => (
        <div
          className="content__img w-[190px] aspect-[1.1] rounded-[15px] absolute top-0 left-0 opacity-0 overflow-hidden pointer-events-none [will-change:transform,filter]"
          key={i}
        >
          <div
