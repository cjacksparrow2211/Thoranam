...
const Loader = ({ placeholderSrc }) => {
  ...
  return (
    <Html center>
      {placeholderSrc ? (
        <img src={placeholderSrc} width={128} height={128} className="blur-lg rounded-lg" alt="Loading 3D Model" />
      ) : (
        <div className="bg-[#121c15]/90 border border-accent/40 backdrop-blur-md px-4 py-2 rounded-full text-accent font-mono text-xs shadow-xl whitespace-nowrap">
          Loading 3D Showcase: {Math.round(progress)}%
        </div>
      )}
    </Html>
  );
...
const ModelViewer = ({
  ...
  return (
    <div
      ...
      className="relative rounded-3xl overflow-hidden shadow-2xl border border-accent/30 bg-[#0c140e]/95"
    >
      {showScreenshotButton && (
        <button
          onClick={capture}
          className="absolute top-4 right-4 z-10 cursor-pointer px-4 py-2 border border-accent/40 rounded-xl bg-primary/80 backdrop-blur-md text-accent text-xs font-semibold hover:bg-accent hover:text-primary transition-colors shadow-lg"
        >
          Capture 3D Frame
        </button>
      )}
